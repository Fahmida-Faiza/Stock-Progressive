...
if(isset($_GET['delete'])){
  $sno= $_GET['delete'];
  $sql= "DELETE FROM `book` WHERE `sno`= $sno";
...
if(isset($_GET['id'])){
    $sno = $_GET['id'];
    $sql = "SELECT * FROM book WHERE sno = $sno";
    $result = mysqli_query($conn, $sql);
...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if(isset($_POST['snoEdit'])){
        ...
        $sql ="UPDATE `book` SET `date` = '$date', `company` = '$company', `price`= '$price' WHERE `book`.`sno` = $sno";
        $result = mysqli_query($conn, $sql);
        ...
        // $buying= $_POST["buying"];
        $price= $_POST["price"];
// `price`, `company`, `book`, `mobile`, `quantity`
        $sql ="INSERT INTO `book` (`date`,`price`, `company`, `book`, `mobile`, `quantity`) VALUES ('$date', '$price', '$company', '$book', '$mobile',  '$quantity')";
        $result = mysqli_query($conn, $sql);
...
<script>
  ...
  Array.from(edits).forEach((element) => {
    element.addEventListener("click", (e) => {
      ...
      let price = tr.getElementsByTagName("td")[5].innerText;
      console.log(date, comapny, model, ram, ime, price);
