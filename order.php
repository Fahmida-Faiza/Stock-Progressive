...
if(isset($_GET['delete'])){
  $sno= $_GET['delete'];
  $sql= "DELETE FROM `book_order` WHERE `sno`= $sno";
...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if(isset($_POST['snoEdit'])){
        ...
        $sql ="UPDATE `book_order` SET `date` = '$date', `company` = '$company', `price`= '$price', `quantity`= '$quantity' WHERE `book_order`.`sno` = $sno";
        $result = mysqli_query($conn, $sql);
        ...
        $sql ="INSERT INTO `book_order` (`date`,`price`, `company`, `book`, `mobile`, `quantity`) VALUES ('$date', '$price', '$company', '$book', '$mobile',  '$quantity')";
        $result = mysqli_query($conn, $sql);
    ...
    <tbody>
      ...
      while($row = mysqli_fetch_assoc($result)){
        ...
        echo "<tr>
          ...
          <td>" . $row['quantity'] . "</td>
          <td>
            <button class='btn btn-primary'><a class='edit' href='order_edit.php?id=".$row['sno']."'>Edit</a></button>
  ...
  document.addEventListener('click', function(e){
      if(e.target && e.target.classList.contains('delete')){
          ...
          if(confirm("Are you sure you want to delete this?")){
              window.location = `/Stock%20Progressive/order.php?delete=${sno}`;
          }
      }
  });
</script>
